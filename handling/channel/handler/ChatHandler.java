    ...
    public static final void GeneralChat(final String text, final byte unk, final MapleClient c, final MapleCharacter chr) {
        if (text.length() > 0 && chr != null && chr.getMap() != null && !CommandProcessor.processCommand(c, text, chr.getBattle() == null ? CommandType.NORMAL : CommandType.POKEMON)) {
            ...
            if (chr.getCanTalk() || chr.isStaff()) {
                ...
                c.getSession().write(MaplePacketCreator.serverNotice(6, "현재 채팅금지가 되어있습니다."));
            }
        }
    }
        ...
        switch (slea.readByte()) {
                    ...
                    if (target != null) {
                        if (target.getMessenger() == null) {
                            if (!target.isIntern() || c.getPlayer().isIntern()) {
                                c.getSession().write(EtcPacket.messengerNote(input, 4, 1));
                                target.getClient().getSession().write(EtcPacket.messengerInvite(c.getPlayer().getName(), messenger.getId()));
                            } else {
                                c.getSession().write(EtcPacket.messengerNote(input, 4, 0));
                            }
                        } else {
                            c.getSession().write(EtcPacket.messengerChat(c.getPlayer().getName(), " : " + target.getName() + " is already using Maple Messenger."));
                        }
                    } else {
            ...
            case 0x05: // decline
                ...
                if (target != null) { // This channel
                    if (target.getMessenger() != null) {
                        target.getClient().getSession().write(EtcPacket.messengerNote(c.getPlayer().getName(), 5, 0));
                    }
                } else { // Other channel
            ...
            case 5: { // Find
                ...
                if (player != null) {
                    if (!player.isIntern() || c.getPlayer().isIntern() && player.isIntern()) {
                        ...
                        c.getSession().write(EtcPacket.getFindReplyWithMap(player.getName(), player.getMap().getId(), mode == 68));
                    } else {
                    ...
                    if (ch > 0) {
                        ...
                        if (player != null) {
                            if (!player.isIntern() || (c.getPlayer().isIntern() && player.isIntern())) {
                                c.getSession().write(EtcPacket.getFindReply(recipient, (byte) ch, mode == 68));
                            } else {
                                c.getSession().write(EtcPacket.getWhisperReply(recipient, (byte) 0));
                            }
                            return;
                    ...
                    if (ch == -10) {
                        c.getSession().write(EtcPacket.getFindReplyWithCS(recipient, mode == 68));
                    } else if (ch == -20) {
            ...
            case 6: { // Whisper
                ...
                if (ch > 0) {
                    ...
                    if (!c.getPlayer().isIntern() && player.isIntern()) {
                        ...
                        c.getSession().write(EtcPacket.getWhisperReply(recipient, (byte) 1));
                    }
                    if (c.isMonitored()) {

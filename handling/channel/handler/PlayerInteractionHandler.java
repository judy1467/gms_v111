            ...
            case CREATE: {
                ...
                if (createType == 3) { // trade
                    ...
                    if (chr.getMap().getMapObjectsInRange(chr.getTruePosition(), 20000, Arrays.asList(MapleMapObjectType.SHOP, MapleMapObjectType.HIRED_MERCHANT)).size() != 0 || chr.getMap().getPortalsInRange(chr.getTruePosition(), 20000).size() != 0) {
                        chr.dropMessage(1, "이 곳 에는 설치하실수 없습니다.");
                        c.getSession().write(MaplePacketCreator.enableActions());
                        return;
                    } else if (chr.getPosition().y >= 890) {
                            chr.dropMessage(1, "고용상인 전용구간에 설치해주시기 바랍니다.");
                            c.getSession().write(MaplePacketCreator.enableActions());
                            return;
                        ...
                        if (createType == IMaplePlayerShop.MATCH_CARD || createType == IMaplePlayerShop.OMOK) {
                            ...
                            if (shop == null || shop.getQuantity() <= 0 || shop.getItemId() != slea.readInt() || c.getPlayer().getMapId() != chr.CMap) {
                                chr.dropMessage(1, "이 곳 에서는 고용상인을 이용하실수 없습니다.");
                                return;
                            }
                            if (createType == 4) {
...
case INVITE_TRADE: {
                ...
                if (chr.getLevel() < 50) {
                    chr.dropMessage(1, "레벨 50 미만은 교환이 불가능합니다.");
                    return;
                }

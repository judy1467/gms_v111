...
public class CashShopOperation {
    ...
    public static void LeaveCS(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            try {
                c.getSession().write(EtcPacket.getChannelChange(InetAddress.getByName(socket[0]), Integer.parseInt(ChannelServer.getInstance(c.getChannel()).getIP().split(":")[1])));
            ...
            //c.getSession().close();//ss펄스저거 니가넣은값아님           아니면.. 뭔가여기가 아닌듯 월드 이동되는게         
            // 닫을 필요가없는이유 세션자체를 사용한게 업어서 대표적으로 오딘이 실수한 부분임 / 갓티썬
        }
    }
    ...
    public static final void BuyCashItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (action == 0) {
            ...
            if (marr != null && marr.getCustomData() != null && Long.parseLong(marr.getCustomData()) >= System.currentTimeMillis()) {
                ...
                chr.dropMessage(1, "펜던트 슬롯 기간이 30일 추가로 확장되었습니다.");
            } else {
                ...
                chr.dropMessage(1, "펜던트 슬롯 개수가 확장되었습니다.");
            }

    ...
    public final void memberLevelJobUpdate(final MapleCharacter mgc) {
        ...
        if (member != null) {
            ...
            if (old_level != mgc.getLevel()) {
                ...
                this.broadcast(MaplePacketCreator.serverNotice(5, "< 패밀리 > " + mgc.getName() + "님이 " + mgc.getLevel() + "레벨을 달성하셨습니다."), mgc.getId(), mgc.getId() == leaderid ? null : member.getPedigree());
            }
            if (old_job != mgc.getJob()) {
                ...
                this.broadcast(MaplePacketCreator.serverNotice(5, "< 패밀리 > " + mgc.getName() + "님이 " + GameConstants.getJobName((int) mgc.getJob()) + "로 전직하셨습니다."), mgc.getId(), mgc.getId() == leaderid ? null : member.getPedigree());
            }
        }
    }

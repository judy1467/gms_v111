        ...
        for (final MapleGuildCharacter member : members) {
            if (member.getId() == mgc.getId()) {
                ...
                if (old_level != mgc.getLevel()) {
                    ...
                    this.broadcast(MaplePacketCreator.serverNotice(5, "< 길드 > " + mgc.getName() + "님이 " + mgc.getLevel() + "레벨을 달성하셨습니다."));
                }
                if (old_job != mgc.getJobId()) {
                    //this.broadcast(CWvsContext.sendJobup(false, mgc.getJobId(), mgc.getName()), mgc.getId());
                    this.broadcast(MaplePacketCreator.serverNotice(5, "< 길드 > " + mgc.getName() + "님이 " + GameConstants.getJobName(mgc.getJobId()) + "로 전직하셨습니다."));
                }

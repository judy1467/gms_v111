...
public class MapleCoconut extends MapleEvent {
	...
	super.onMapLoad(chr);
        chr.getClient().getSession().write(EtcPacket.coconutScore(getCoconutScore()));
    }
                ...
                if (getMapleScore() == getStoryScore()) {
                    ...
                    for (MapleCharacter chr : getMap(0).getCharactersThreadsafe()) {
                        if (chr.getTeam() == (getMapleScore() > getStoryScore() ? 0 : 1)) {
                            chr.getClient().getSession().write(EtcPacket.showEffect("event/coconut/victory"));
                            chr.getClient().getSession().write(EtcPacket.playSound("Coconut/Victory"));
                        } else {
                            chr.getClient().getSession().write(EtcPacket.showEffect("event/coconut/lose"));
                            chr.getClient().getSession().write(EtcPacket.playSound("Coconut/Failed"));
                        }
                    }
                    warpOut();
                }

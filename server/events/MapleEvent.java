...
public abstract class MapleEvent {
    ...
    public static void givePrize(final MapleCharacter chr) {
        ...
        if (reward == 0) {
            ...
            chr.addFame(70);
            chr.dropMessage(5, "축하합니다. 인기도 70을 얻으셨습니다.");
	} else if (reward == 4) {
	    chr.dropMessage(5, "꽝~~ 아무런 보상이 없네요.");
        } else {
            int max_quantity = 1;
            switch (reward) {
                case 5062000:
            ...
            chr.dropMessage(5, "랜덤아이템을 얻으셨습니다. 인벤토리를 확인하세요.");
	...
	if (GameConstants.isEventMap(chr.getMapId()) && FieldLimitType.Event.check(chr.getMap().getFieldLimit()) && FieldLimitType.Event2.check(chr.getMap().getFieldLimit())) {
	    chr.getClient().getSession().write(EtcPacket.showEventInstructions());
	}
    }

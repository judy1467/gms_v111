...
public class PokemonBattle {
    ...
    public void initiate(MapleCharacter initiator, PokemonMap mapp) { //wild battle
        ...
        initiator.getClient().getSession().write(EtcPacket.musicChange(wildBGM));
    ...
    public void initiate(MapleCharacter initiator, MapleNPC npc) { //wild battle
        ...
        initiator.dropMessage(-6, "What will you do? @use @info @ball @run @battlehelp");
        turn[1] = makeRandomTurn();
        initiator.getClient().getSession().write(EtcPacket.musicChange(npcBGM));
        firstTurn(battlers[0], battlers[1]);
    }
    ...
    public void initiate() { //trainer battle
        ...
        initiator.getClient().getSession().write(EtcPacket.musicChange(trainerBGM));
        initiator2.getClient().getSession().write(EtcPacket.musicChange(trainerBGM));
        firstTurn(battlers[0], battlers[1]);
                    ...
                    EtcTimer.getInstance().schedule(new Runnable() {
                        ...
                        public void run() {
                            if (user != null && !disposed) {
                                user.getClient().getSession().write(EtcPacket.playSound("Cokeplay/Fall"));
                                catchEffect(battlers[1].getMonster());
                            } else {
                                dispose();
                            }
                    ...
                    EtcTimer.getInstance().schedule(new Runnable() {
                        ...
                        public void run() {
                            if (user != null && !disposed) {
                                ...
                                user.getClient().getSession().write(EtcPacket.playSound("Romio/discovery"));
                                    ...
                                    public void run() {
                                        ...
                                        if (battlers[i == 1 ? 0 : 1].getCurrentHP() <= 0) {
                                                        ...
                                                        if (b != null && b.getMonsterId() == z) {
                                                            ...
                                                            if (b.getTrueLevel() > oLevel) {
                                                                ...
                                                                if (!b.getStats().getName().equals(oName)) {
                                                                    ch.getClient().getSession().write(EtcPacket.playSound("5th_Maple/prize"));
                                            ...
                                            if (instanceid >= 0 && i == 0 && npcTeam == null) {
                                                ...
                                                chr.getClient().getSession().write(EtcPacket.playSound("Romio/discovery"));
                                                dispose(chr, false);
                                                return;
                                            }
                                            if (instanceid >= 0 && i == 0 && npcTeam != null) {

...
public class Event_PyramidSubway {
	...
	bar = 0;
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null) {
                    chr.getClient().getSession().write(EtcPacket.getClock(time));
                    chr.getClient().getSession().write(EtcPacket.showEffect("killing/first/number/" + stage));
                    chr.getClient().getSession().write(EtcPacket.showEffect("killing/first/stage"));
                    chr.getClient().getSession().write(EtcPacket.showEffect("killing/first/start"));
                    fullUpdate(chr, stage);
                }
            }
        } else {
            c.getClient().getSession().write(EtcPacket.getClock(time));
            c.getClient().getSession().write(EtcPacket.showEffect("killing/first/number/" + stage));
            c.getClient().getSession().write(EtcPacket.showEffect("killing/first/stage"));
            c.getClient().getSession().write(EtcPacket.showEffect("killing/first/start"));
            fullUpdate(c, stage);
        }
	...
	c.getTrait(MapleTraitType.will).addExp((type + 2) * 8, c);
        c.getClient().getSession().write(EtcPacket.showEffect("killing/clear"));
        c.getClient().getSession().write(EtcPacket.sendPyramidResult(rank, exp));
        dispose(c);
    }
    ...
    public final void broadcastUpdate(final MapleCharacter c) {
        c.getClient().getSession().write(EtcPacket.sendPyramidUpdate(bar));
    }

    public final void broadcastEffect(final MapleCharacter c, final String effect) {
        c.getClient().getSession().write(EtcPacket.showEffect(effect));
    }
    ...
    private static final void changeMap(final MapleCharacter c, final MapleMap map, final int minLevel, final int maxLevel, final int clear) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null && chr.getId() != c.getId() && chr.getLevel() >= minLevel && chr.getLevel() <= maxLevel) {
                    if (clear == 1) {
                        chr.getClient().getSession().write(EtcPacket.showEffect("killing/clear"));
                    } else if (clear == 2) {
                        chr.getClient().getSession().write(EtcPacket.showEffect("killing/fail"));
                    }
        ...
        if (clear == 1) {
            ...
            c.getClient().getSession().write(EtcPacket.showEffect("killing/fail"));
        }

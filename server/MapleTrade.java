...
public class MapleTrade {
    ...
    public static final void startTrade(final MapleCharacter c) {
        if (c.getTrade() == null) {
            ...
            c.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "이미 교환 중 입니다."));
        }
	...
	if (c1 == null || c1.getTrade() == null) {
        ...
        if (c2 != null && c2.getTrade() == null) {
            ...
            c1.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "대상이 이미 다른 플레이어와 교환을 하고있습니다."));
    ...
    public static final void visitTrade(final MapleCharacter c1, final MapleCharacter c2) {
	    ...
	    c1.getTrade().inTrade = true;
            ...
            c1.dropMessage(-2, "[System] : 교환불가 아이템은 아래와 같은 방식으로 등록해주세요. <@소비등록, @설치등록, @기타등록, @캐시등록> <갯수> <아이템이름>");
            c1.dropMessage(-2, "[System] : 예시 : '@캐시등록 1 고성능 확성기' , '@소비등록 4 빨간 포션");
            c2.dropMessage(-2, "[System] : 교환불가 아이템은 아래와 같은 방식으로 등록해주세요. <@소비등록, @설치등록, @기타등록, @캐시등록> <갯수> <아이템이름>");
            c2.dropMessage(-2, "[System] : 예시 : '@캐시등록 1 고성능 확성기' , '@소비등록 4 빨간 포션");
        } else {
            c1.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "교환이 성사되지 않았습니다."));
        }
    }
    ...
    public static final void declineTrade(final MapleCharacter c) {
		...
		if (other != null && other.getTrade() != null) {
                    ...
                    other.setTrade(null);
                    other.dropMessage(5, c.getName() + "님이 교환신청을 거절하였습니다.");
		}
            }

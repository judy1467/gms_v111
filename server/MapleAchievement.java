...
public class MapleAchievement {
    ...
    public void finishAchievement(MapleCharacter chr) {
        ...
        if (notice && !chr.isGM()) {
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[알림] " + (GameConstants.GMS ? (reward/2) : reward) + "캐시 를 획득하였습니다.."));
        } else {
            chr.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "[알림] " + (GameConstants.GMS ? (reward/2) : reward) + "캐시 를 획득하였습니다.."));
        }
    }
}

...
public class MapleInventoryManipulator {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!GameConstants.isRechargable(item.getItemId())) {
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner()) && item.getExpiration() == eItem.getExpiration()) {
                                ...
                                quantity -= (newQ - oldQ);
                                eItem.setQuantity(newQ);
                                c.getSession().write(InventoryPacket.updateInventorySlot(type, eItem, hm));
                            }
                        } else {
                            break;
                        }
                ...
                while (quantity > 0) {
                    ...
                    c.getSession().write(InventoryPacket.addInventorySlot(type, nItem, hm));
                }
            } else {
                // Throwing Stars and Bullets - Add all into one slot regardless of quantity.
            ...
            if (quantity == 1) {
                ...
                c.getSession().write(InventoryPacket.addInventorySlot(type, item, hm));
                if (GameConstants.isHarvesting(item.getItemId())) {
                    c.getPlayer().getStat().handleProfessionTool(c.getPlayer());
                }
            } else {
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (source.getItemId() / 10000 == 166) {
            ...
            if(target2 == null) {
                c.getPlayer().dropMessage(1, "안드로이드 심장아이템을 장착 해야 사용이가능합니다.");
                c.getSession().write(MaplePacketCreator.enableActions());
                return;
            }
        ...
        } else if (source.getItemId() == 1122121) { //오토루팅 아이템 코드
            if (chr.getAutoLoot() == false) {
                chr.setAutoLoot(true);
            }
           chr.dropMessage(6, "오토루팅 아이템 착용으로 오토루팅이 활성화됩니다.");
        }
            ...
            for (int i : potentials) {
                if (i > 0) {
                    ...
                    if (pot != null && pot.get("skillID") > 0) {
                        chr.dropMessage(5, SkillFactory.getSkillName(pot.get("skillID")) +" 스킬이 활성화되었습니다.");
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        if(source.getAndroid2()) {
            ...
            if(target2 != null) {
                c.getPlayer().dropMessage(1, "안드로이드부터 벗겨라");
                c.getSession().write(MaplePacketCreator.enableActions());
                return;
            }
        }
        ...
        if (source.getState() >= 17) {
            ...
            for (int i : potentials) {
                if (i > 0) {
                    StructItemOption pot = ii.getPotentialInfo(i).get(ii.getReqLevel(source.getItemId()) / 10);
                    if (pot != null && pot.get("skillID") > 0) {
                        c.getPlayer().dropMessage(5, SkillFactory.getSkillName(pot.get("skillID")) +" 스킬이 비활성화되었습니다.");

        ...
        try {
            ...
            for (MapleCharacter chr : getPlayers()) {
				if (name.startsWith("PVP")) {
					chr.getClient().getSession().write(EtcPacket.getPVPClock(Integer.parseInt(getProperty("type")), timesend));
				} else {
					chr.getClient().getSession().write(EtcPacket.getClock(timesend));
				}
            }
            timeOut(time, this);

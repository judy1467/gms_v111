...
public abstract class AbstractPlayerInteraction {
    ...
    public final void warp(final int map, final int portal) {
        ...
        if (portal != 0 && map == c.getPlayer().getMapId()) { //test
            ...
            if (portalPos.distanceSq(getPlayer().getTruePosition()) < 90000.0) { //estimation
                c.getSession().write(EtcPacket.instantMapWarp((byte) portal)); //until we get packet for far movement, this will do
    ...
    public final void warp(final int map, String portal) {
        ...
        if (map == c.getPlayer().getMapId()) { //test
            ...
            if (portalPos.distanceSq(getPlayer().getTruePosition()) < 90000.0) { //estimation
                c.getPlayer().checkFollow();
                c.getSession().write(EtcPacket.instantMapWarp((byte) c.getPlayer().getMap().getPortal(portal).getId()));
        ...
        if (quantity >= 0) {
            ...
            if (type.equals(MapleInventoryType.EQUIP) && !GameConstants.isThrowingStar(id) && !GameConstants.isBullet(id)) {
                ...
                if (id / 10000 == 114 && name != null && name.length() > 0) { //medal
                    final String msg = "<" + name + "> 칭호를 획득 하셨습니다.";
	...
	c.getSession().write(InfoPacket.getShowItemGain(id, quantity, true));
    }
    ...
    public final void dojo_getUp() {
        ...
        c.getSession().write(EtcPacket.instantMapWarp((byte) 6));
    }
    ...
    public final void showInstruction(final String msg, final int width, final int height) {
        c.getSession().write(EtcPacket.sendHint(msg, width, height));
    }
    ...
    public final void Aran_Start() {
        c.getSession().write(EtcPacket.Aran_Start());
    }
    ...
    public final void EarnTitleMsg(final String data) {
        c.getSession().write(MaplePacketCreator.getTopMsg(data));
    }
    ...
    public void showMapEffect(String path) {
        getClient().getSession().write(EtcPacket.MapEff(path));
    }
...
//	FileoutputUtil.log(FileoutputUtil.PQ_Log, text);
    ...
    public void trembleEffect(int type, int delay) {
        c.getSession().write(EtcPacket.trembleEffect(type, delay));
    }
   ...
   public void setKeyValue(String key, String value) {
    ...
    public void warpParty(int id, boolean isNext) {
       ...
       if (mmap.getCharactersSize() > 0) {
           c.getPlayer().dropMessage(1, "이미 다른 파티가 입장하였습니다. 잠시후 시도하세요.");
           return;
       }
   ...
   public final int getAndroidStat(final String type) { 
    ...
    public void updateChar() {
        getPlayer().getClient().getSession().write(EtcPacket.getCharInfo(getPlayer()));
